...
from core.preprocess import RGBMaskRCNNTorchPreprocess
...
class RGBMaskRCNNTorchServicer(vision_interfaces_pb2_grpc.DetectClassifyItemServicer):
    def __init__(self, node_config_file: str):
        ...
        self.preprocessor = RGBMaskRCNNTorchPreprocess(self.cfg.INPUT.MIN_SIZE_TEST, 
    ...
    def preprocess(self, request):
        ...
        # get roi in the format of (w, h)
        rois = reinterpret_roi(self.input_data.roi, self.input_data.camera_data.color_data.shape[:-1][::-1])
        self.preprocessor.rois = rois
        self.input_image = self.preprocessor.get_input_image(self.input_data.camera_data.color_data)
